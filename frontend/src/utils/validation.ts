...
const allowedDependantRelations = new Set(['spouse', 'son', 'daughter'])
...
export const validateDependant = (payload: DependantPayload): string | null => {
  ...
  if (!allowedDependantRelations.has(payload.relation.trim().toLowerCase())) {
    return 'Relation must be Spouse, Son, or Daughter.'
  }
